...
unsigned long __dl_runtime_pltresolve(struct elf_resolve *tpnt, int reloc_entry)
{
...
void _dl_perform_tsar_global_got_relocations(struct elf_resolve *tpnt, int lazy)
{
	ElfW(Sym) *sym;
	char *strtab;
...
#if defined (__SUPPORT_LD_DEBUG__)
		if (_dl_debug_reloc)
			_dl_dprintf(2, "_dl_perform_tsar_global_got_relocations for '%s'\n", tpnt->libname);
#endif
